...
<div class="content mt-3">
    ...
    <div class="card">
        <div class="card-body">
            <div class="float-right mb-2">
                <div class="btn-group">
                    <button class="btn btn-info" data-toggle="modal" data-target="#addcron"><i class="fa fa-plus"></i>
                        add
                    </button>
                </div>
            </div>
                ...
                if ($count > 0) {
                    ...
                    <tr <?php if ($row->active == 0) { ?> class="bg-light"<?php } ?>>
                        ...
                        <td class="text-center">
                            <p data-field="active" id="active" class="cronactive nounderline" data-input="select"
                               data-id="<?= $row->id; ?>"><?php if ($row->active == 0) { ?>Inactive<?php }
                                if ($row->active == 1) { ?>Active <?php } ?></p></td>
                        <td><p data-field="name" class="cronname nounderline txt text-center" data-input="input"
                               data-id="<?= $row->id; ?>"
                               data-title="Rename Cron ID <?= $row->id; ?>"><?= $row->name; ?></p></td>
                        <td><p data-field="file" class="cronfile nounderline txt text-center" data-input="input"
                               data-id="<?= $row->id; ?>"
                               data-title="Change File for <?= $row->name; ?>"><?= $row->file; ?></p></td>
                        <td><p data-field="sort" class="cronsort nounderline txt text-center" data-input="input"
                               data-id="<?= $row->id; ?>"
                               data-title="Change sort for <?= $row->name; ?>"><?= $row->sort; ?></p></td>
                        ...
                        <td class="text-center">
                            ...
                            if ($ranCount > 0) {
                                ...
                                <?= $ranResult->datetime; ?> (<?= echousername($ranResult->user_id); ?>)<?php } else { ?>
                                <i>Never</i><?php } ?></td>
                        <td class="text-center">
                            <button type="button" name="button" id="deleteCron" data-value="<?= $row->id ?>"
    ...
    <?php if ($settings->cron_ip == 'off') { ?>
        ...
        Step 1: Go into your server and set your cron job to fire off to <?= $_SERVER['REQUEST_SCHEME'] . '://' . $_SERVER['HTTP_HOST'] . $us_url_root . 'users/cron/cron.php' ?> every few minutes.
        <br>
        Step 2: Go into <a href="admin.php?view=logs">the system
            logs</a> and see which ip address was rejected for trying to do a cron job.<br>
        Step 3: Then go into <a href="admin.php?view=general">the admin
            dashboard"</a> and set that IP address in the 'Only allow cron jobs from the following IP' box.<br>
            ...
            <div class="modal-content">
                ...
                <form class="form-signup" action="" method="POST">
                    <div class="modal-body">
                        ...
                        <label>Cron Name: </label><input type="text" class="form-control" id="name" name="name"
                                                         placeholder="Cron Name" required>

                        <label>File: </label><input type="text" class="form-control" id="file" name="file"
                                                    placeholder="File (include type, e.g. .php) within the cron folder only"
                                                    required>

                        <label>Sort: </label><input type="text" class="form-control" id="sort" name="sort"
                                                    placeholder="3 digit sort code, crons run by this order, eg 100, 101, 102"
                                                    required>
                        <br/>
                    </div>
                    <div class="modal-footer">
                        <div class="btn-group">
                            ...
                            <input class='btn btn-info mr-2' type='submit' name="addCron" value='Add Cron'
                                   class='submit'/>
