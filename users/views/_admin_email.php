...
<div class="content mt-3">
    <form name='update' action='' method='post'>
        ...
        <p class="text-dark">
            These settings control all things email-related for the server including emailing your users and verifying
            the user's email address.
            You must obtain and verify all settings below for YOUR email server or hosting provider. Encryption with TLS
            is STRONGLY recommended,
            followed by SSL. No encryption is like shouting your login credentials out into a crowded field and is not
            supported for now.
        </p>
        <p class="text-dark">It is <strong>HIGHLY</strong> recommended that you test your email settings before turning
            on the feature to require new users to verify their email</p>
                ...
                <div class="card no-padding">
                    <div class="card-body">
                        ...
                        <div class="form-group">
                            ...
                            <input size='50' class='form-control' type='text' autocomplete="off" name='website_name'
                                   value='<?= $results->website_name; ?>'/>
                        </div>
                        <div class="form-group">
                            <label>SMTP Server:</label>
                            <input size='50' class='form-control' type='text' autocomplete="off" name='smtp_server'
                                   value='<?= $results->smtp_server; ?>'/>
                        </div>
                        <div class="form-group">
                            <label>SMTP Port:</label>
                            <input size='50' class='form-control' type='text' autocomplete="off" name='smtp_port'
                                   value='<?= $results->smtp_port; ?>'/>
                        </div>
                        <label>Email Login/Username:</label>
                        <input size='50' class='form-control' type='password' autocomplete="off" name='emx'
                               value='<?= $results->email_login; ?>'/>
                        <div class="form-group">
                            <label>Email Password:</label>
                            <input size='50' class='form-control' type='password' autocomplete="off" name='emp'
                                   value='<?= $results->email_pass; ?>'/>
                        </div>
                        <div class="form-group">
                            <label>From Name (For Sent Emails):</label>
                            <input size='50' class='form-control' type='text' autocomplete="off" name='from_name'
                                   value='<?= $results->from_name; ?>'/>
                        </div>
                        <div class="form-group">
                            <label>From Email (For Sent Emails):</label>
                            <input size='50' class='form-control' type='text' autocomplete="off" name='frome'
                                   value='<?= $results->from_email; ?>'/>
                        </div>
                        <div class="form-group">
                            <label>PHPMailer Authtype: (Typically CRAM-MD5, LOGIN, PLAIN, or XOAUTH2)</label>
                            <input size='50' class='form-control' type='text' autocomplete="off" name='authtype'
                                   value='<?= $results->authtype; ?>'/>
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" name="transport">
                                ...
                                <option value="ssl" <?php if ($results->transport == 'ssl') {
                                    echo 'selected="selected"';
                                } ?> >SSL (encrypted, but weak)
                                </option>
                            </select>
                        </div>
                        <div class="form-group">
                            <label>Email Debugging Level <a href="#!" tabindex="-1" title="Note" data-trigger="focus"
                                                            class="nounderline" data-toggle="popover" data-content="0=Off, 1=Client Messages, 2=Normal Debug, 3=More Verbose, 4=Extremely Verbose.
                Debugging should be off in production projects for security reasons"><i
                        ...
                        <div class="form-group">
                            <label>Use isSMTP Feature <a href="#!" tabindex="-1" title="Note" data-trigger="focus"
                                                         class="nounderline" data-toggle="popover"
                                                         data-content="Use this if your email keeps failing and you know your credentials are correct."><i
                                            class="fa fa-question-circle"></i></a></label>
                        ...
                        <div class="form-group">
                            <label>Root URL of your UserSpice install <a href="#!" tabindex="-1" title="Note"
                                                                         data-trigger="click" class="nounderline"
                                                                         data-toggle="popover"
                                                                         data-content="Including http or https protocol (VERY Important). Default location would be: <?= $urlProtocol . $_SERVER['HTTP_HOST'] . $us_url_root; ?>"><i
                                            class="fa fa-question-circle"></i></a></label>
                            <small class="form-text text-muted">
                                Put http://yourdomain.com/ with the final / below
                            </small>
                            <input size='50' class='form-control' type='text' name='verify_url'
                                   value='<?= $results->verify_url; ?>'/>
                        </div>
                        <div class="form-group">
                            <label for="email_act">Require User to Verify Their Email</label>
                            <label for="email_act_yes"><input type="radio" id="email_act_yes" name="email_act"
                                                              value="1" <?php echo ($results->email_act == 1) ? 'checked' : ''; ?>
                                                              size="25"/>&nbsp;Yes</label>&nbsp;
                            <label for="email_act_no"><input type="radio" id="email_act_no" name="email_act"
                                                             value="0" <?php echo ($results->email_act == 0) ? 'checked' : ''; ?>
                                                             size="25"/>&nbsp;No</label>
                        </div>
                        <input type="hidden" name="csrf" value="<?= Token::generate(); ?>"/>
                        <div class="text-center">
                            <input class='btn btn-primary' name="update_only" type='submit'
                                   value='Update Email Settings' class='submit'/>
                            <input class='btn btn-danger' name="update_and_test" type='submit'
                                   value='Update and Test Email Settings' class='submit'/>
                        </div>
                    </div>
                </div>
            </div>
