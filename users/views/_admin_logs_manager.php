...
<div class="content mt-3">
    <h2>Logs Manager <a class="nounderline" href="../users/admin.php?view=logsman"><i
                    class="fa fa-fw fa-refresh"></i></a> <a class="nounderline" href="../users/admin_logs.php"><i
                    class="fa fa-fw fa-search"></i></a></h2>
    <hr>
    <center>
        <div>
            <table class="table table-bordered">
                ...
                <tr>
                    <th>
                        <center>Log Type</center>
                    </th>
                    <th>
                        <center>Count</center>
                    </th>
                    <th>
                        <center>Exempted?</center>
                    </th>
                    <th>
                        <center>Mapper Function</center>
                    </th>
                </tr>
                <?php
                if ($count > 0) {
                    foreach ($query->results() as $row) { ?>
                        ...
                        <td>
                            <center><a href="#" data-name="exempt" id="exempt" data-value="<?= $exp ?>"
                                       class="exempt nounderline" data-mode="popup" data-type="select"
                                       data-pk="<?= $row->logtype; ?>" data-url="admin_logs_exempt.php"
                                       data-title="Do you wish to exempt logs for <?= $row->logtype; ?>?"><?php if ($exempt->count() > 0) { ?>Yes<?php } else { ?> No<?php } ?></a>
                            </center>
                        </td>
                        <td>
                            <center><a href="#" data-name="mapper" id="mapper" class="mapper nounderline"
                                       data-mode="popup" data-type="select" data-pk="<?= $row->logtype; ?>"
                                       data-url="admin_logs_mapper.php"
                                       data-title="What would you like to map <?= $row->logtype; ?> as?">Map</a>
                            </center>
                        </td>
                        </tr><?php
                    }
                ...
                <form class="form-signup" action="logsman.php" method="POST">
                    ...
                    <div class="modal-footer">
                        <div class="btn-group"><input type="hidden" name="csrf" value="<?= Token::generate(); ?>"/>
                            <input class='btn btn-info' type='submit' name="addLog" value='Add Exemption'
                                   class='submit'/></div>
                </form>
                <div class="btn-group">
