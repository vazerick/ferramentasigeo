...
<div class="content mt-3">
    ...
    <div class="btn-group pull-right">
        <button type="button" class="btn btn-info" data-toggle="modal" data-target="#composemass"><i
                    class="fa fa-plus"></i> New Mass Notification
        </button>
    </div>
    <br><br>
            ...
            <?php if ($count > 0) { ?>
                <?php foreach ($adminNotifications as $m) { ?>
                  ...
                  <span class="chat-img pull-left" style="padding:5px">
                    <img src="<?= get_gravatar($db->query("SELECT email FROM users WHERE id = $m->user_id")->first()->email) ?>"
                         width="75" class="img-thumbnail">
                  </span>
                        </td>
                        <td><input type="checkbox" class="maincheck" name="checkbox[<?= $m->id ?>]"
                                   value="<?= $m->id ?>"/> <?= echouser($m->user_id) ?>
                            , <?= time2str($m->date_created) ?><br/><?= html_entity_decode($m->message) ?>
                            <br/>
                            <?php if ($m->is_read == 1 && $m->is_archived == 0) { ?><i
                                    class="fa fa-check"></i> Read<?php }
                            if ($m->is_read == 0 && $m->is_archived == 0) { ?> <i
                                    class="fa fa-times"></i> Delivered<?php }
        ...
        <?php if ($count > 0) { ?>
            <table class="table pull-right" width="20%">
                <tr>
                    ...
                    <td width="5%">
                        <input class='btn btn-primary' type='submit' name="admin_notifications" value='Go!'
                               class='submit'/></td>
                </tr>
            </table>
        <?php } ?>
    ...
    <div class="modal-dialog">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form autocomplete="off" name="create_mass_message" action="admin.php?view=admin_notifications"
                      method="post">
                    <label>Class:</label>
