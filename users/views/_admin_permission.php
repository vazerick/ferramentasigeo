    ...
    <form name='adminPermission' action='' method='post'>
        <div class="row">
            <div class="col-sm-12">
                ...
                <p>
                    <strong>Note:</strong>This permission id is used in functions like <strong>hasPerm</strong> and is
                    how you refer to the permission when coding.
                </p>
            </div>
        </div>
        <div class="row">
            <div class="col-md-6">
                ...
                <div class="form-check">
                    <input class="form-check-input" type="checkbox" value="<?= $permissionDetails['id'] ?>"
                           id="delete[<?= $permissionDetails['id'] ?>]" name='delete[<?= $permissionDetails['id'] ?>]'>
                    <label class="form-check-label" for="delete[<?= $permissionDetails['id'] ?>]">
                        Delete Level <?= $permissionDetails['name'] ?> (<strong
                                class="text-default">irreversable</strong>)
                    </label>
                </div>
            </div>
            <div class="col-md-6">
                ...
                <div class="input-group">
                    ...
                    <input type="text" class="form-control" aria-describedby="name-label" name="name"
                           value='<?= $permissionDetails['name'] ?>' required>
                </div>
        ...
        <div class="row">
            ...
            <div class="col-md-2 mt-2">
                <div class="nav flex-column nav-pills" id="v-pills-tab" role="tablist" aria-orientation="vertical">
                    <a class="nav-link active" id="ManageUsers" data-toggle="pill" href="#ManageUsersTab" role="tab"
                       aria-controls="ManageUsersTab" aria-selected="true">Manage Users</a>
                    <a class="nav-link" id="RemovePages" data-toggle="pill" href="#RemovePagesTab" role="tab"
                       aria-controls="RemovePagesTab" aria-selected="false">Remove Pages</a>
                    <a class="nav-link" id="AddPages" data-toggle="pill" href="#AddPagesTab" role="tab"
                       aria-controls="AddPagesTab" aria-selected="false">Add Pages</a>
                    <a class="nav-link" id="PublicPages" data-toggle="pill" href="#PublicPagesTab" role="tab"
                       aria-controls="PublicPagesTab" aria-selected="false">Public Pages</a>
                </div>
            ...
            <div class="col-md-10 mt-2">
                <div class="tab-content" id="v-pills-tabContent">
                    <div class="tab-pane fade show active" id="ManageUsersTab" role="tabpanel"
                         aria-labelledby="ManageUsers">
                        <h3>Manage Users</h3>
                        <form class="" action="" method="get">
                            ...
                            <div class="input-group">
                                ...
                                <input type="text" name="query" class="form-control"
                                       placeholder="Search by ID, First Name, Last Name, Username, or Email">
                  ...
                  <input type="submit" name="Submit" value="Search" class="btn btn-primary">
                                        ...
                                        <td>
                                            ...
                                            if (!in_array($r->id, $master_account)) {
                                                ...
                                                <form class="" action="" method="post">
                                                    ...
                                                    if ($count > 0) { ?>
                                                        <input type="submit" name="permChange" value="Remove"
                                                               class="btn btn-danger btn-block">
                                                    <?php } else { ?>
                                                        <input type="submit" name="permChange" value="Add"
                    ...
                    <div class="tab-pane fade" id="RemovePagesTab" role="tabpanel" aria-labelledby="RemovePages">
                        ...
                        foreach ($pageData as $v1) {
                            if (in_array($v1->id, $page_ids)) { ?>
                                <br><label class="normal"><input type='checkbox' name='removePage[]' class='removePage'
                                                                 id='removePage[]'
                                                                 value='<?= $v1->id; ?>'> <?= $v1->page; ?></label>
                            <?php }
                        } ?>
                    </div>
                    <div class="tab-pane fade" id="AddPagesTab" role="tabpanel" aria-labelledby="AddPages">
                        ...
                        foreach ($pageData as $v1) {
                            if ($settings->page_permission_restriction == 1) {
                                ...
                                if (!in_array($v1->id, $page_ids) && $v1->private == 1 && !$countCountQ >= 1) { ?>
                                    <br><label class="normal"><input type='checkbox' name='addPage[]' class='addPage'
                                                                     id='addPage[]'
                                                                     value='<?= $v1->id; ?>'> <?= $v1->page; ?></label>
                                <?php }
                            } else {
                        ...
                        <p><br><strong>Private - Cannot Be Assigned</strong>
                            ...
                            foreach ($pageData as $v1) {
                                ...
                                $countCountQ = $countQ->count();
                                if (!in_array($v1->id, $page_ids) && $v1->private == 1 && $countCountQ >= 1) { ?>
                                    <br><?= $v1->page; ?> (<?php if ($countCountQ > 1) { ?>Multiple<?php } else { ?><a
                                        href="admin.php?view=page&id=<?= $v1->id ?>"
                                        style="text-decoration:none;"><?= fetchPermissionDetails($countQ->first()->permission_id)['name'] ?></a><?php } ?>)
                                <?php }
                            } ?>
                    ...
                    <div class="tab-pane fade" id="PublicPagesTab" role="tabpanel" aria-labelledby="PublicPages">
                        ...
                        foreach ($pageData as $v1) {
                            if ($v1->private != 1) {
                                ?><br><a href="admin.php?view=page&id=<?= $v1->id ?>"
                                         style="text-decoration:none;"><?= $v1->page ?></a>
                            <?php }
                        }
                        ?>
                    </div>
