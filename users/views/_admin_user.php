...
<div class="content mt-3">
    ...
    <form class="form" id='adminUser' name='adminUser' action='' method='post'>
        <div class="row">
            <div class="col-8">
                <h3><span id="fname"><?= $userdetails->fname; ?> </span><span
                            id="lname"><?= $userdetails->lname; ?> </span><span id="slash">- </span><span
                            id="username"><?= $userdetails->username; ?></span></h3>
                <label>User ID: </label> <?= $userdetails->id; ?><?php if ($act == 1) { ?> <br>
                    <?php if ($userdetails->email_verified == 1) { ?> Email Verified <input type="hidden"
                                                                                            name="email_verified"
                                                                                            value="1"/>
        ...
        <div class="row">
            <div class="col-12 col-sm-6">
                <div class="form-group" id="username-group">
                    ...
                    <input class='form-control' type='search' name='unx' value='<?= $userdetails->username; ?>'
                           autocomplete="off"/>
                </div>

                <div class="form-group" id="email-group">
                    <label>Email:</label>
                    <input class='form-control' type='search' name='emx' value='<?= $userdetails->email; ?>'
                           autocomplete="off"/>
                </div>

                <div class="form-group" id="fname-group">
                    <label>First Name:</label>
                    <input class='form-control' type='search' name='fnx' value='<?= $userdetails->fname; ?>'
                           autocomplete="off"/>
                </div>

                <div class="form-group" id="lname-group">
                    <label>Last Name:</label>
                    <input class='form-control' type='search' name='lnx' value='<?= $userdetails->lname; ?>'
                           autocomplete="off"/>
                </div>

                <div class="form-group">
                    <label>New Password (<?= $settings->min_pw; ?> char min, <?= $settings->max_pw; ?> max.)</label>
                    <input class='form-control' type='password' autocomplete="off" name='pwx'
                           <?php if ((!in_array($user->data()->id, $master_account) && in_array($userId, $master_account) || !in_array($user->data()->id, $master_account) && $userdetails->protected == 1) && $userId != $user->data()->id) { ?>disabled<?php } ?>/>
                </div>

                <div class="form-group">
                    <label>Confirm Password</label>
                    <input class='form-control' type='password' autocomplete="off" name='confirm'
                           <?php if ((!in_array($user->data()->id, $master_account) && in_array($userId, $master_account) || !in_array($user->data()->id, $master_account) && $userdetails->protected == 1) && $userId != $user->data()->id) { ?>disabled<?php } ?>/>
                </div>


                <div class="form-group">
                    <label><input type="checkbox" name="sendPwReset" id="sendPwReset"/> Send Reset Email? Will expire
                        in <?= $settings->reset_vericode_expiry; ?> minutes.</label><br>
                </div>
                <?php includeHook($hooks, 'form'); ?>
                <div class="row">
                    <div class="col-12 col-sm-6">
                        <div class="panel-heading"><strong>Remove These
                                Permission(s)</strong> <?php if ($protectedprof == 1) { ?><p class="pull-right">
                                PROTECTED PROFILE - EDIT DISABLED</p><?php } ?></div>
                            ...
                            foreach ($permissionData as $v1) {
                                if (in_array($v1->id, $perm_ids)) { ?>
                                    <label class="normal"><input type='checkbox' name='removePermission[]'
                                                                 id='removePermission[]' value='<?= $v1->id; ?>'
                                                                 <?php if (!hasPerm([$v1->id], $user->data()->id) && $settings->permission_restriction == 1) { ?>disabled<?php } ?> /> <?= $v1->name; ?>
                                    </label><br>
                                    <?php
                                }
                            }
                    ...
                    <div class="col-12 col-sm-6">
                        <div class="panel-heading"><strong>Add These
                                Permission(s)</strong> <?php if ($protectedprof == 1) { ?><p class="pull-right">
                                PROTECTED PROFILE - EDIT DISABLED</p><?php } ?></div>
                        <div class="panel-body">
                            <?php
                            foreach ($permissionData as $v1) {
                                if (!in_array($v1->id, $perm_ids)) { ?>
                                    <label class="normal"><input type='checkbox' name='addPermission[]'
                                                                 value='<?= $v1->id; ?>'
                                                                 <?php if (!hasPerm([$v1->id], $user->data()->id) && $settings->permission_restriction == 1) { ?>disabled<?php } ?>/> <?= $v1->name; ?>
                                    </label><br>
                                    <?php
            ...
            <div class="col-12 col-sm-6">
                <div class="form-group">
                    <label> Is allowed to cloak<a class="nounderline" data-toggle="tooltip"
                                                  title="Warning: This is an extremely powerful permission and should not be given lightly!!!"><span
                                    style="color:blue">?</span></a></label>
                ...
                <div class="form-group">
                    <label> Block<a class="nounderline" data-toggle="tooltip"
                                    title="Drop the banhammer on a troublemaker!"><span style="color:blue">?</span></a></label>
                ...
                <div class="form-group">
                    <label> Force Password Reset<a class="nounderline" data-toggle="tooltip"
                                                   title="The user will be required to create a new password on next login"><span
                                    style="color:blue">?</span></a></label>
                ...
                <div class="form-group">
                    <label>Dev User<a class="nounderline" data-toggle="tooltip"
                                      title="This is just a flag that you can set for your own purposes.  It will be accessable from $user->data()->dev_user"><span
                                    style="color:blue">?</span></a></label>
                ...
                <div class="form-group">
                    ...
                    <label>Cloak into this user<a class="nounderline" data-toggle="tooltip"
                                                  title="Automatically logs you in as this user"><span
                ...
                <div class="form-group">
                    <label>Delete this User<a class="nounderline" data-toggle="tooltip"
                                              title="Completely delete a user. This cannot be undone."><span
                                    style="color:blue">?</span></a></label>
                    <select name='delete[<?php echo "$userId"; ?>]' id='delete[<?php echo "$userId"; ?>]'
                            class="form-control">
