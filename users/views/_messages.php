...
<div class="content mt-3">
    ...
    <h2>Conversations <a href="#" data-toggle="modal" class="nounderline" data-target="#settings"><i
                    class="fa fa-cog"></i></a></h2>
    <?php if ($count > 0) { ?><label><input type="checkbox" class="checkAllMsg"/>
        [ check/uncheck all ]</label><?php } ?>
    <div class="btn-group pull-right">
        <button type="button" class="btn btn-info" data-toggle="modal" data-target="#composemass"><i
                    class="fa fa-plus"></i> New Mass Message
        </button>
    </div>
        ...
        <center>
            <table id="paginate" class="table table-striped">
                ...
                <tr>
                    ...
                    <td class="pull-left">
                        <h4>
                            <input type="checkbox" class="maincheck" name="checkbox[<?= $m->id ?>]"
                                   value="<?= $m->id ?>"/>
                            <a class="nounderline" href="admin_message.php?id=<?= $m->id ?>">
                                <?= $m->msg_subject ?> - between <?= echouser($m->msg_from); ?>
                                and <?= echouser($m->msg_to); ?>
                            </a>
                    ...
                    <td>
                        <span class="pull-left"><i
                                    class="fa fa-<?php if ($m->archive_from == 0) { ?>times<?php } else { ?>check<?php } ?>"></i> Archived by <?= echouser($m->msg_from) ?> (f)</span>
                        <span class="pull-right"><i
                                    class="fa fa-<?php if ($m->archive_to == 0) { ?>times<?php } else { ?>check<?php } ?>"></i> Archived by <?= echouser($m->msg_to) ?> (t)</span><br/>
                        <hr>
                        <span class="pull-left"><i
                                    class="fa fa-<?php if ($m->hidden_from == 0) { ?>times<?php } else { ?>check<?php } ?>"></i> Deleted by <?= echouser($m->msg_from) ?> (f)</span>
                        <span class="pull-right"><i
                                    class="fa fa-<?php if ($m->hidden_to == 0) { ?>times<?php } else { ?>check<?php } ?>"></i> Deleted by <?= echouser($m->msg_to) ?> (t)</span>
                    </td>
                </tr>
                <?php }
                } else { ?>
                    <td colspan="3">
                        <center><h3>No Conversations</h3></center>
                    </td></tr>
                <?php } ?>
                </tbody>
            </table>
            ...
            <table class="table pull-right" width="20%">
                <tr>
                    ...
                    <td width="5%">
                        ...
                        <input class='btn btn-primary' type='submit' name="admin_messages" value='Go!' class='submit'/>
                    </td>
                </tr>
            </table>
        <?php } ?>
    ...
    <div class="modal-dialog">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form autocomplete="off" class="form" id="messageSettings" name='messageSettings'
                      action='admin.php?view=messages' method='post'>
                    <div class="form-group">
                        ...
                        <select id="msg_notification" class="form-control" name="msg_notification">
                            <option value="1" <?php if ($settings->msg_notification == 1) echo 'selected="selected"'; ?> >
                                Enabled
                            </option>
                            <option value="0" <?php if ($settings->msg_notification == 0) echo 'selected="selected"'; ?> >
                                Disabled
                            </option>
                        </select>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select id="msg_blocked_users" class="form-control" name="msg_blocked_users">
                            <option value="1" <?php if ($settings->msg_blocked_users == 1) echo 'selected="selected"'; ?> >
                                Enabled
                            </option>
                            <option value="0" <?php if ($settings->msg_blocked_users == 0) echo 'selected="selected"'; ?> >
                                Disabled
                            </option>
                        </select>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select id="msg_default_to" class="form-control" name="msg_default_to">
                            <option value="1" <?php if ($settings->msg_default_to == 1) echo 'selected="selected"'; ?> >
                                Yes
                            </option>
                            <option value="0" <?php if ($settings->msg_default_to == 0) echo 'selected="selected"'; ?> >
                                No
                            </option>
                        </select>
                    </div>
            ...
            <div class="modal-footer">
                <div class="btn-group">
                    <input type="hidden" name="csrf" value="<?= $csrf; ?>"/>
                    <input class='btn btn-primary' type='submit' name="messageSettings" value='Update' class='submit'/>
                </div>
                </form>
                <div class="btn-group">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form autocomplete="off" name="create_mass_message" action="admin.php?view=messages" method="post">
                    ...
                    <label><input type="checkbox" name="sendEmail" id="sendEmail" checked/> Send Email Notification if
                        Enabled?</label>
                    <input type="hidden" name="csrf" value="<?= $csrf; ?>"/>
                    </p>
                    <p>
            ...
            <div class="modal-footer">
                <div class="btn-group">
                    <input type="hidden" name="csrf" value="<?= $csrf; ?>"/>
                    <input class='btn btn-primary' type='submit' name="send_mass_message" value='Send Message'
                           class='submit'/></div>
                </form>
                <div class="btn-group">
                    <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
