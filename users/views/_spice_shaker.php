...
<div class="content mt-3">
    ...
    <h2>Spice Shaker Auto Installer
        <?php if (!$diag) { ?>
            <button type="button" onclick="window.location.href = 'admin.php?view=spice&diag=1';" name="button"
                    class="btn btn-primary">Enter Diagnostic Mode
            </button>
        <?php } else { ?>
            <button type="button" onclick="window.location.href = 'admin.php?view=spice';" name="button"
                    class="btn btn-primary">Leave Diagnostic Mode
            </button>
        <?php } ?>
    </h2>
    <?php if ($diag) { ?>
        <br><h3>Please Install/Update the demo plugin and look at the messages above and in your logs to diagnose API
            issues.</h3><br>
    <?php } ?>
    Spice Shaker allows you to download and automatically install Updates, Plugins, Templates, Widgets, and Languages
    for UserSpice.<br>Users with a (free) API key can make 2000 requests a day.<br>
    <?php
    $failed = 0;
            ...
            <div class="col-12 col-sm-4">
                <form class="" action="" method="post">
                    <div class="input-group">
                        <input type="text" name="search" class="form-control" value="" placeholder="Search all addons"
            ...
            <form class="" action="" method="post">
                ...
                <div class="d-flex">
                    <select class="form-control" name="type">
                        <option value="" disabled <?php if ($type == ''){ ?>selected="Selected"<?php } ?>>--Choose
                            One--
                        </option>
                        <option value="featured" <?php if ($type == 'featured'){ ?>selected="Selected"<?php } ?>>
                            Featured
                        </option>
                        <option value="plugin" <?php if ($type == 'plugin'){ ?>selected="Selected"<?php } ?>>Plugins
                        </option>
                        <option value="template" <?php if ($type == 'template'){ ?>selected="Selected"<?php } ?>>
                            Templates
                        </option>
                        <option value="widget" <?php if ($type == 'widget'){ ?>selected="Selected"<?php } ?>>Widgets
                        </option>
                        <option value="translation" <?php if ($type == 'translation'){ ?>selected="Selected"<?php } ?>>
                            Languages
                        </option>
                    </select>
...
<?php } else { ?>
    <h2>You must enter your Free UserSpice API key <a href="admin.php?view=general">here</a> in order to use this
        feature.</h2>
<?php } ?>
                    ...
                    <div class="card-footer" style="background: inherit; border-color: inherit;">
                        ...
                        if (shakerIsInstalled($d->category, $d->reserved)) {
                            if ($d->category == "plugin" && file_exists($abs_us_root . $us_url_root . "usersc/plugins/" . $d->reserved . "/.noupdate")) {
                                ...
                                ?>
                                <button type="button" name="button" class="btn btn-danger installme <?= $warning ?>"
                                        data-res="<?= $d->reserved ?>" data-type="<?= $d->category ?>"
                                        data-url="<?= $d->dd ?>" data-hash="<?= $d->hash ?>"
                                        data-counter="<?= $counter ?>">Update
                                </button>
                                <?php
                            } //end noupdate
                        } else { ?>
                            <button type="button" name="button" class="btn btn-primary installme <?= $warning ?>"
                                    data-res="<?= $d->reserved ?>" data-type="<?= $d->category ?>"
                                    data-url="<?= $d->dd ?>" data-hash="<?= $d->hash ?>" data-counter="<?= $counter ?>">
                                Download
                            </button>
                        <?php } ?>
                        <a href="https://github.com/<?= $d->repo ?>/tree/master/src/<?= $d->reserved ?>"
                           class="btn btn-outline-primary" target="_blank">View Source</a>
                        <a href="#" class="btn btn-success visit" target="_blank" style="display:none"
                           id="<?= $counter ?>">Check it Out!</a>
                    </div>
                </div>
