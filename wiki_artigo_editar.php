...
$db->query("SELECT * FROM wiki_artigos WHERE id = '" . $_GET["id"] . "'");

if ((count($db->results()) == 0)) {
    echo "ARTIGO NÃO LOCALIZADO";
    die();
}
...
if (!(hasPerm($artigo->grupo))) {
    echo "USUÁRIO NÃO AUTORIZADO PARA VER ESTE ARTIGO";
    die();
}
...
function submit(){
    ...
    if (count($ErrorArrays) == 0) {
        ...
        if ($db->error()) {
            ...
            header('Location: wiki_artigo_editar.php?id=' . $_GET["id"]);
        }
...
<?php tinymce($artigo->titulo, $artigo->conteudo, $pastas, $equipes); ?>

<script>
    document.getElementById("setor").value = <?php echo $artigo->grupo; ?>;
    document.getElementById("pasta").value = <?php echo $artigo->pasta; ?>;
    //function carregar(){
    //    document.getElementById("setor").value = <?php //echo $artigo->grupo; ?>//;
    //    document.getElementById("pasta").value = <?php //echo $artigo->pasta; ?>//;
    //}
